...
class TestPlanAgent:
    async def test_build_prompt(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cmd_dir = Path(tmpdir) / ".cursor" / "commands"
            cmd_dir.mkdir(parents=True, exist_ok=True)
            (cmd_dir / "adk:plan.md").write_text("Plan with $ARGUMENTS", encoding="utf-8")
            ctx = AgentContext(
                db=None,
                session_id="s1",
                entity_id="t1",
            ...
            assert ("PRD content" in prompt) or ("Build feature X" in prompt)
    ...
    async def test_on_complete_raises_when_plan_missing(self):
        config = get_agent_config("plan")
        with tempfile.TemporaryDirectory() as tmpdir:
            task = SimpleNamespace(id="t1", tech_plan="")
            db = AsyncMock()
...
class TestTodoExecAgent:
    async def test_build_prompt(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            cmd_dir = Path(tmpdir) / ".cursor" / "commands"
            cmd_dir.mkdir(parents=True, exist_ok=True)
            (cmd_dir / "adk:implement.md").write_text("Implement $ARGUMENTS", encoding="utf-8")
...
class TestReviewAgent:
    async def test_not_runnable(self):
