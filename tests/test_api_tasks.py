...
_mock_generate_spec = patch("daiflow.routers.tasks.generate_spec", new_callable=AsyncMock)


class TestSpecRoute:
    """Tests for POST /api/tasks/{id}/spec endpoint.

    Requires:
    - daiflow.services.task_service.generate_spec function
    - POST /api/tasks/{id}/spec route in routers/tasks.py
    - task_service imported as generate_spec in routers/tasks.py
    """

    @_mock_bg
    @_mock_generate_spec
    async def test_trigger_spec_success_in_planning_state(self, mock_spec, mock_init, client, db_session):
        """POST /spec succeeds when task is in PLANNING state."""
        from daiflow.models import Task, TaskStatus

        pid = await _create_project(client)
        create_resp = await client.post("/api/tasks", json={"name": "Task 1", "project_id": pid})
        tid = create_resp.json()["id"]

        # Set task to PLANNING state
        task = await db_session.get(Task, tid)
        task.status = TaskStatus.PLANNING
        await db_session.commit()

        resp = await client.post(f"/api/tasks/{tid}/spec")

        assert resp.status_code == 200
        data = resp.json()
        assert data.get("ok") is True or "ok" in data

    @_mock_bg
    @_mock_generate_spec
    async def test_trigger_spec_wrong_state_returns_400(self, mock_spec, mock_init, client, db_session):
        """POST /spec should return 400 when task is not in a valid state for spec."""
        from daiflow.models import Task, TaskStatus
        ...
        # Task is in CREATED (0), not PLANNING
        resp = await client.post(f"/api/tasks/{tid}/spec")
        assert resp.status_code == 400

    async def test_trigger_spec_not_found_returns_404(self, client):
        """POST /spec returns 404 for unknown task id."""
        resp = await client.post("/api/tasks/nonexistent_task_xyz/spec")
        assert resp.status_code == 404

    @_mock_bg
    @_mock_generate_spec
    async def test_trigger_spec_schedules_background_task(self, mock_spec, mock_init, client, db_session):
        """generate_spec should be scheduled as a background task when POST /spec is called."""
        from daiflow.models import Task, TaskStatus
        ...
        await client.post(f"/api/tasks/{tid}/spec")

        # generate_spec must have been called (as background task)
        mock_spec.assert_called()
        # Called with the task id
        call_args = mock_spec.call_args
        assert tid in str(call_args)

    @_mock_bg
    async def test_task_response_includes_spec_doc_field(self, mock_init, client, db_session):
        """GET /tasks/{id} response must include spec_doc field (even if empty)."""
        pid = await _create_project(client)
        create_resp = await client.post("/api/tasks", json={"name": "Task 1", "project_id": pid})
        tid = create_resp.json()["id"]
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert "spec_doc" in data, (
            "spec_doc field missing from TaskResponse schema. "
            "Add spec_doc: str = '' to TaskResponse in schemas.py"
        )

    @_mock_bg
    @_mock_generate_spec
    async def test_trigger_spec_allowed_in_planning(self, mock_spec, mock_init, client, db_session):
        """POST /spec is allowed when task is in PLANNING state (primary valid state)."""
        from daiflow.models import Task, TaskStatus
