...
class TestTaskWorkflow:
    ...
    async def test_reset_init_from_initializing(self, db_session: AsyncSession):
        p = Project(name="proj")
        db_session.add(p)
        await db_session.flush()
    ...
    async def test_reset_plan_stays_in_planning(self, db_session: AsyncSession):
        p = Project(name="proj")
        db_session.add(p)
        await db_session.flush()
        t = Task(name="t", project_id=p.id, status=TaskStatus.PLANNING)
    ...
    async def test_reset_todos_to_planning(self, db_session: AsyncSession):
        p = Project(name="proj")
        db_session.add(p)
        await db_session.flush()
    ...
    async def test_reset_review_to_coding(self, db_session: AsyncSession):
        p = Project(name="proj")
        db_session.add(p)
        await db_session.flush()
        t = Task(name="t", project_id=p.id, status=TaskStatus.REVIEWING)
        db_session.add(t)
        await db_session.flush()
