...
class TestRunAgent:
    ...
    async def test_marks_session_failed_on_error(self, db_session):
        ...
        with (
            ...
            patch("daiflow.agent_executor.build_task_runner", side_effect=Exception("cody build failed")),
        ):
            await run_agent(
                db_session,
                agent_type="plan",
    ...
    async def test_ref_id_for_todo_exec(self, db_session):
        ...
        with (
            ...
            patch("daiflow.agent_executor.build_task_runner", side_effect=Exception("stop")),
        ):
            await run_agent(
                db_session,
    ...
    async def test_full_successful_run(self, db_session):
        ...
        with (
            patch("daiflow.agent_executor.get_task_context", new_callable=AsyncMock, return_value=(None, ["/repo"])),
            patch("daiflow.agent_executor.build_task_runner", new_callable=AsyncMock, return_value=mock_client),
