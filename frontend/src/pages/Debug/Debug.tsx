...
import { listProjects, listTasks, listSessions, getSessionLogs, getTodos, listJobs, getJobRuns, forceFailSession, syncSessionStatus } from '../../api'
              ...
              <>
                ...
                {selectedSessionObj && (
                  <div className="debug-session-meta">
                    ...
                    {(selectedSessionObj.status === 0 || selectedSessionObj.status === 1) && (
                      <div className="meta-row">
                        <button
                          className="debug-force-fail-btn"
                          onClick={async () => {
                            if (!confirm(t('debug.force_fail_confirm'))) return
                            try {
                              const updated = await forceFailSession(selectedSessionObj.session_id)
                    ...
                    {selectedSessionObj.status === 3 && ['plan', 'todo_split', 'todo_exec', 'review'].includes(selectedSessionObj.type) && (
                      <div className="meta-row">
                        <button
                          className="debug-force-fail-btn"
                          onClick={async () => {
                            if (!confirm(t('debug.sync_status_confirm'))) return
                            try {
                              const res = await syncSessionStatus(selectedSessionObj.session_id)
                              alert(res.message)
                            } catch (err) {
                              alert(String(err))
                            }
...
function toPreviewString(v: unknown): string {
  if (v == null) return ''
...
function LogEntry({ entry, index }: { entry: Record<string, unknown>; index: number }) {
  ...
  const preview = type === 'text_delta'
    ? toPreviewString(entry.content).slice(0, 120)
    : type === 'tool_call'
    ? (entry.name as string) || (entry.tool as string) || ''
    : type === 'error'
    ? toPreviewString(entry.content ?? entry.error ?? entry.message).slice(0, 120)
    : ''
