...
export function useCodingStage(taskId: string | undefined) {
  ...
  // Optimistic flag: set true immediately on executeAll() before first loadData confirms
  const [optimisticRunAll, setOptimisticRunAll] = useState(false)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setTodos(td)
      // Once API confirms run_all_in_progress, the optimistic flag is no longer needed
  ...
  // A todo_exec session exists only after execution starts.
  // Avoid polling /api/sessions for pending/skipped todos, which would return 404.
  const hasTodoExecSession = !!currentTodo && (
    currentTodo.status === TodoStatus.RUNNING ||
    currentTodo.status === TodoStatus.DONE ||
    currentTodo.status === TodoStatus.FAILED ||
    !!currentTodo.cody_session_id
  )
  const sessionId = (hasTodoExecSession && currentTodo && taskId)
    ? sessionIds.todoExec(taskId, currentTodo.id)
    : null
  ...
  useEffect(() => {
    ...
    const runningTodo = todos.find(t => t.status === TodoStatus.RUNNING)
    if (!runningTodo) return
    if (selectedTodo === runningTodo.id) return
    // Only auto-switch when current selection is not itself RUNNING
    const currentSelected = todos.find(t => t.id === selectedTodo)
    if (!currentSelected || currentSelected.status !== TodoStatus.RUNNING) {
      setSelectedTodo(runningTodo.id)
    }
  }, [todos, isRunAllInProgress, selectedTodo])
