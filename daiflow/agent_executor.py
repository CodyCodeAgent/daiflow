...
async def run_agent(
    ...
    try:
        ...
        # 4. Build runner (honours task/project/global runner config)
...
async def prepare_chat(
    ...
    # Fallback: if config did not resolve one, reuse from current Session (e.g. plan chat)
    elif cody_session_id is None:
        session_rec = await db.get(Session, session_id)
        if session_rec and session_rec.cody_session_id:
            cody_session_id = session_rec.cody_session_id
    ...
    return StageChatContext(
        ...
        cody_client=agent_runner,   # StageChatContext.cody_client now holds AbstractAgentRunner
        cody_session_id=cody_session_id,
        on_tool_result=on_tool_result,
        language=lang,
